...
复现 [*Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware*](https://arxiv.org/abs/2304.13705)（ACT, RSS 2023）在 MuJoCo 上的两个双臂任务。训练与评估均在单卡 RTX 4090 D 上完成，论文配置 2000 epochs。

| 任务 | 结果 | 论文 | 视频 |
|---|---|---|---|
| `sim_transfer_cube_scripted` 双臂传递方块 | **49/50 = 98.0%** | 86% | [普通做法](sim_transfer_cube_scripted/final_ep2000/rollout00_no_temporal_agg_success_return685.mp4) ｜ [时间集成](sim_transfer_cube_scripted/final_ep2000/rollout00_temporal_agg_success_return640.mp4) |
...
插入任务要求插片推进到槽内 pin 才算成功，50 条失败里 30 条卡在"碰到插座但没插进去"，所以失败录像看上去很像成功。
